// AI Agent Simulator for Live Telemetry

import { AgentStatus, AgentState, JobPhase, TelemetryEvent } from '@/types/telemetry';
...
export class AgentSimulator {
  ...
  private randomState(): AgentState {
    ...
    const weights = [0.2, 0.5, 0.2, 0.1]; // running is most common
    return this.weightedRandom(states, weights);
  }

  private randomJob() {
    if (Math.random() < 0.3) return undefined; // 30% chance of no current job
    ...
    return {
      ...
      startedAt: new Date(Date.now() - Math.random() * 300000).toISOString(), // up to 5 min ago
    };
  }
  ...
  private generateTokenStats() {
    ...
    const cost = total * 0.000002; // $2 per 1M tokens
  ...
  private generatePerformanceStats() {
    ...
    return {
      latencyMsP50: p50,
      latencyMsP95: p95,
      cacheHitRate: parseFloat((Math.random() * 0.6 + 0.1).toFixed(2)), // 10-70%
      retries: Math.floor(Math.random() * 5),
      errors5xx: Math.floor(Math.random() * 3),
      toolCalls: Math.floor(Math.random() * 10) + 1,
    };
  }

  private generateContextStats() {
    return {
      utilization: parseFloat((Math.random() * 0.8 + 0.1).toFixed(2)), // 10-90%
      promptLength: Math.floor(Math.random() * 3000) + 500,
      compressionLevel: Math.floor(Math.random() * 4), // 0-3
      toolsInConversation: Math.floor(Math.random() * 5) + 1,
    };
  }

  private generateSafetyStats() {
    const flagged = Math.random() < 0.1; // 10% chance of being flagged
    const labels = ['inappropriate_content', 'violence', 'hate_speech', 'spam'];
    
    return {
      flagged,
      lastLabel: flagged ? labels[Math.floor(Math.random() * labels.length)] : undefined,
  ...
  private startSimulation() {
    // Update agent states every 2-5 seconds
  ...
  private updateAgentStates() {
    this.agents.forEach((agent, id) => {
      ...
      if (Math.random() < 0.1) {
        ...
        if (newState !== agent.state) {
          ...
          this.emitEvent({
            ...
            type: newState === 'error' ? 'error' : 'job_started',
            agentId: id,
            jobId: agent.currentJob?.id,
          });
        }
      }

      // Update job phase (20% chance)
      ...
      // Update queue (30% chance)
      if (Math.random() < 0.3) {
        agent.queue = Math.max(0, agent.queue + (Math.random() < 0.5 ? -1 : 1));
      }
    });
  }
  ...
  private generateRandomEvent() {
    ...
    const eventTypes: TelemetryEvent['type'][] = [
      'job_started', 'job_finished', 'tool_call', 'cache_hit', 'error', 'safety_block'
    ];
    ...
    this.emitEvent({
      ...
      type: eventType,
      agentId,
      jobId: this.agents.get(agentId)?.currentJob?.id,
  ...
  private updateMetrics() {
    this.agents.forEach(agent => {
      ...
      agent.performance.cacheHitRate = Math.max(0, Math.min(1, 
        agent.performance.cacheHitRate + (Math.random() * 0.1 - 0.05)
      ));

      // Update context utilization
      agent.context.utilization = Math.max(0, Math.min(1,
        agent.context.utilization + (Math.random() * 0.1 - 0.05)
      ));
    });
  }
...
export function getAgentSimulator(): AgentSimulator {
  if (!simulatorInstance) {
    simulatorInstance = new AgentSimulator();
  }
